...
function auth_login() {
	...
	if($_SESSION[ SYSTEM_NAME . 'type'] == 'admin' && dirname($_SERVER['SCRIPT_NAME']) !=  DIRECTORY . ADMIN ) 
		$flag = false;   
	if($_SESSION[ SYSTEM_NAME . 'type'] == 'authority' && dirname($_SERVER['SCRIPT_NAME'])   !=  DIRECTORY . AUTHORITY ) 
		$flag = false;  
	if($_SESSION[ SYSTEM_NAME . 'type'] == 'inmate' && dirname($_SERVER['SCRIPT_NAME'])   !=  DIRECTORY . INMATE ) 
		$flag = false;  
...
function auth_use() {
	...
	if( isset( $_SESSION[ SYSTEM_NAME . 'userid'] )   ) {  
		...
		switch (user_type()) {
			...
			case 'authority':
			setLocation(DIRECTORY_AUTHORITY);
			break; 

			
			case 'inmate':
			setLocation(DIRECTORY_INMATE);
			break; 
